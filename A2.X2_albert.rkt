...
(define (closure name)
  (list
   ; Point env to current next
   
   ; Create pair
   ; Store in temp, or assembly compiler complains
   (movq (label-reference name) temp)
   (movq temp (★ next 0))
   (movq env (★ next 1))
   (movq next result)
   ; Add 16 to next (double)
...
(define (variable n)
   (define loop (build-list n values))
   (define body (map (λ (_) (movq (★ temp  0) temp)) loop))
   (append '() (list (movq env temp) body (movq (★ temp 1) result)))
  )
...
(define (set n)
  (define loop (build-list n values))
  (define body (map (λ (_) (movq (★ temp  0) temp)) loop))
  (append '() (list (movq env temp) body (movq result (★ temp 1)))
  ))
