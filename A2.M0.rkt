...
; ------
; Outside scope of the assignment: for fun
...
(define-transformer T:Σ Σ
  (`(Σ ,i ,start ,end ,e ..1)
   `(let ([result 0] [,i ,start])
      (while (<= ,i ,end)
            (set! result (+ result . ,e))
            (set! ,i (+ ,i 1)))
      result)))
...
(define-transformer T:λ λ
  [`(λ (,id) ,body ..1) `(L0: λ (,id) (block . ,body))]
  [`(λ () ,body ..1) `(λ (_) . ,body)]
  [`(λ (,id ...) ,body ..1)
   `(λ (,(first id)) ,(append `(λ ,(rest id)) body))])
...
(define-transformer T:block block
  [`(block) 0]
  [`(block ,e) e]
  [`(block ,e ...) `(let ([_ ,(first e)]) ,(append '(block) (rest e)))])
...
(module+ test
  (check-equal? ((transformer-function T:cond) '(cond [0 99]))
                '(when 0
                   99))
